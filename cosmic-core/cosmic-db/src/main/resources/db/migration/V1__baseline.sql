...
CREATE TABLE affinity_group_domain_map (
  ...
  FOREIGN KEY (affinity_group_id) REFERENCES affinity_group (id)
    ON DELETE CASCADE
);
...
CREATE TABLE autoscale_policy_condition_map (
  ...
  FOREIGN KEY (policy_id) REFERENCES autoscale_policies (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (autoscale_vmgroup_id) REFERENCES autoscale_vmgroups (id)
  ON DELETE CASCADE;

ALTER TABLE autoscale_vmgroup_policy_map
  ADD CONSTRAINT fk_autoscale_vmgroup_policy_map__vmgroup_id
FOREIGN KEY (vmgroup_id) REFERENCES autoscale_vmgroups (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (profile_id) REFERENCES autoscale_vmprofiles (id);

ALTER TABLE autoscale_vmprofile_details
  ADD CONSTRAINT fk_autoscale_vmprofile_details__autoscale_vmprofile_id
FOREIGN KEY (autoscale_vmprofile_id) REFERENCES autoscale_vmprofiles (id)
  ON DELETE CASCADE;
...
CREATE TABLE cluster (
  ...
  cluster_type     VARCHAR(64) DEFAULT 'Managed'      NULL,
  ...
  COMMENT 'Is this cluster managed by Cosmic',
  removed          DATETIME                           NULL
  COMMENT 'date removed if not null',
...
FOREIGN KEY (default_zone_id) REFERENCES data_center (id)
  ON DELETE CASCADE;

ALTER TABLE autoscale_vmgroups
  ADD CONSTRAINT fk_autoscale_vmgroups__zone_id
FOREIGN KEY (zone_id) REFERENCES data_center (id);
...
CREATE TABLE disk_offering_details (
  ...
  FOREIGN KEY (offering_id) REFERENCES disk_offering (id)
    ON DELETE CASCADE
);
...
CREATE TABLE external_load_balancer_devices (
  ...
  COMMENT '1 if load balancer appliance is provisioned and its life cycle is managed by by Cosmic',
  host_id                    BIGINT                         NOT NULL
  COMMENT 'host id coresponding to the external load balancer device',
  parent_host_id             BIGINT                         NULL
  COMMENT 'if the load balancer appliance is Cosmic managed, then host id on which this appliance is provisioned',
...
CREATE TABLE firewall_rules (
  ...
  FOREIGN KEY (related) REFERENCES firewall_rules (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (firewall_rule_id) REFERENCES firewall_rules (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (gslb_rule_id) REFERENCES global_load_balancing_rules (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (category_id) REFERENCES guest_os_category (id)
  ON DELETE CASCADE;
...
CREATE TABLE guest_os_hypervisor (
  ...
  FOREIGN KEY (guest_os_id) REFERENCES guest_os (id)
);
...
CREATE TABLE image_store_details (
  ...
  FOREIGN KEY (store_id) REFERENCES image_store (id)
    ON DELETE CASCADE
);
...
CREATE TABLE instance_group_vm_map (
  ...
  FOREIGN KEY (group_id) REFERENCES instance_group (id)
    ON DELETE CASCADE
);
...
CREATE TABLE load_balancer_healthcheck_policy_details (
  ...
  FOREIGN KEY (lb_policy_id) REFERENCES load_balancer_healthcheck_policies (id)
    ON DELETE CASCADE
);
...
CREATE TABLE load_balancer_stickiness_policy_details (
  ...
  FOREIGN KEY (lb_policy_id) REFERENCES load_balancer_stickiness_policies (id)
    ON DELETE CASCADE
);
...
CREATE TABLE load_balancing_rules (
  ...
  FOREIGN KEY (id) REFERENCES firewall_rules (id)
    ON DELETE CASCADE
);

ALTER TABLE autoscale_vmgroups
  ADD CONSTRAINT fk_autoscale_vmgroup__load_balancer_id
FOREIGN KEY (load_balancer_id) REFERENCES load_balancing_rules (id)
  ON DELETE CASCADE;

ALTER TABLE global_load_balancer_lb_rule_map
  ADD CONSTRAINT fk_lb_rule_id
FOREIGN KEY (lb_rule_id) REFERENCES load_balancing_rules (id)
  ON DELETE CASCADE;
...
CREATE TABLE mshost_peer (
  ...
  FOREIGN KEY (owner_mshost) REFERENCES mshost (id)
    ON DELETE CASCADE,
  CONSTRAINT fk_mshost_peer__peer_mshost
  FOREIGN KEY (peer_mshost) REFERENCES mshost (id)
);
...
CREATE TABLE network_acl_details (
  ...
  FOREIGN KEY (network_acl_id) REFERENCES network_acl (id)
    ON DELETE CASCADE
);
...
CREATE TABLE network_acl_item (
  ...
  FOREIGN KEY (acl_id) REFERENCES network_acl (id)
    ON DELETE CASCADE
);
...
CREATE TABLE network_acl_item_cidrs (
  ...
  FOREIGN KEY (network_acl_item_id) REFERENCES network_acl_item (id)
    ON DELETE CASCADE
);
...
CREATE TABLE network_external_lb_device_map (
  ...
  FOREIGN KEY (external_load_balancer_device_id) REFERENCES external_load_balancer_devices (id)
    ON DELETE CASCADE
);
...
CREATE TABLE networks (
  ...
  COMMENT 'Cosmic managed vms get IP address from cidr.In general this cidr also serves as the network CIDR. But in case IP reservation feature is being used by a Guest network, networkcidr is the Effective network CIDR for that network',
  ...
  FOREIGN KEY (related) REFERENCES networks (id)
    ON DELETE CASCADE,
...
CREATE TABLE nicira_nvp_nic_map (
  ...
  COMMENT 'Cosmic uuid of the nic connected to this logical switch port',
...
CREATE TABLE nicira_nvp_router_map (
  ...
  network_id         BIGINT       NOT NULL
  COMMENT 'Cosmic id of the network',
...
FOREIGN KEY (nic) REFERENCES nics (uuid)
  ON DELETE CASCADE;
...
CREATE TABLE op_dc_storage_network_ip_address (
  ...
  FOREIGN KEY (range_id) REFERENCES dc_storage_network_ip_range (id)
    ON DELETE CASCADE
);
...
CREATE TABLE op_dc_vnet_alloc (
  ...
  FOREIGN KEY (account_vnet_map_id) REFERENCES account_vnet_map (id)
);
...
CREATE TABLE op_ha_work (
  ...
  FOREIGN KEY (mgmt_server_id) REFERENCES mshost (msid),
...
CREATE TABLE op_host_transfer (
  ...
  FOREIGN KEY (initial_mgmt_server_id) REFERENCES mshost (msid),
  CONSTRAINT fk_op_host_transfer__future_mgmt_server_id
  FOREIGN KEY (future_mgmt_server_id) REFERENCES mshost (msid)
);
...
FOREIGN KEY (physical_network_id) REFERENCES physical_network (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (portable_ip_range_id) REFERENCES portable_ip_range (id)
  ON DELETE CASCADE;
...
CREATE TABLE project_account (
  ...
  FOREIGN KEY (project_account_id) REFERENCES account (id)
    ON DELETE CASCADE
);
...
CREATE TABLE remote_access_vpn_details (
  ...
  FOREIGN KEY (remote_access_vpn_id) REFERENCES remote_access_vpn (id)
    ON DELETE CASCADE
);
...
CREATE TABLE router_network_ref (
  ...
  FOREIGN KEY (router_id) REFERENCES domain_router (id)
    ON DELETE CASCADE,
...
CREATE TABLE s2s_customer_gateway_details (
  ...
  FOREIGN KEY (s2s_customer_gateway_id) REFERENCES s2s_customer_gateway (id)
    ON DELETE CASCADE
);
...
CREATE TABLE s2s_vpn_connection (
  ...
  FOREIGN KEY (customer_gateway_id) REFERENCES s2s_customer_gateway (id)
    ON DELETE CASCADE,
...
CREATE TABLE s2s_vpn_connection_details (
  ...
  FOREIGN KEY (s2s_vpn_connection_id) REFERENCES s2s_vpn_connection (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (vpn_gateway_id) REFERENCES s2s_vpn_gateway (id)
  ON DELETE CASCADE;
...
CREATE TABLE s2s_vpn_gateway_details (
  ...
  FOREIGN KEY (s2s_vpn_gateway_id) REFERENCES s2s_vpn_gateway (id)
    ON DELETE CASCADE
);
...
CREATE TABLE security_group_rule (
  ...
  FOREIGN KEY (allowed_network_id) REFERENCES security_group (id)
    ON DELETE CASCADE
);
...
CREATE TABLE service_offering (
  ...
  FOREIGN KEY (id) REFERENCES disk_offering (id)
    ON DELETE CASCADE
);
...
CREATE TABLE snapshot_policy_details (
  ...
  FOREIGN KEY (policy_id) REFERENCES snapshot_policy (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (certificate_id) REFERENCES sslcerts (id)
  ON DELETE CASCADE;
...
CREATE TABLE storage_pool (
  ...
  COMMENT 'Should Cosmic manage this storage',
  capacity_iops         BIGINT                          NULL
  COMMENT 'IOPS Cosmic can provision from this storage pool',
  CONSTRAINT uuid
  UNIQUE (uuid),
...
CREATE TABLE storage_pool_details (
  ...
  FOREIGN KEY (pool_id) REFERENCES storage_pool (id)
    ON DELETE CASCADE
);
...
CREATE TABLE upload (
  ...
  FOREIGN KEY (host_id) REFERENCES image_store (id)
    ON DELETE CASCADE
);
...
CREATE TABLE usage_event_details (
  ...
  FOREIGN KEY (usage_event_id) REFERENCES usage_event (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (autoscale_user_id) REFERENCES user (id)
  ON DELETE CASCADE;
...
CREATE TABLE user_ip_address (
  ...
  FOREIGN KEY (source_network_id) REFERENCES networks (id),
...
FOREIGN KEY (ip_address_id) REFERENCES user_ip_address (id);

ALTER TABLE remote_access_vpn
  ADD CONSTRAINT fk_remote_access_vpn__vpn_server_addr_id
FOREIGN KEY (vpn_server_addr_id) REFERENCES user_ip_address (id);

ALTER TABLE s2s_vpn_gateway
  ADD CONSTRAINT fk_s2s_vpn_gateway__addr_id
FOREIGN KEY (addr_id) REFERENCES user_ip_address (id)
  ON DELETE CASCADE;
...
CREATE TABLE user_ip_address_details (
  ...
  FOREIGN KEY (user_ip_address_id) REFERENCES user_ip_address (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (instance_id) REFERENCES user_vm (id)
  ON DELETE CASCADE;
...
CREATE TABLE vgpu_types (
  ...
  FOREIGN KEY (gpu_group_id) REFERENCES host_gpu_groups (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (element_id) REFERENCES virtual_router_providers (id);
...
FOREIGN KEY (vlan_db_id) REFERENCES vlan (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (instance_id) REFERENCES vm_instance (id);

ALTER TABLE console_proxy
  ADD CONSTRAINT fk_console_proxy__id
FOREIGN KEY (id) REFERENCES vm_instance (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (vmId) REFERENCES vm_instance (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (vm_template_id) REFERENCES vm_template (id);
...
CREATE TABLE vm_work_job (
  ...
  FOREIGN KEY (vm_instance_id) REFERENCES vm_instance (id)
    ON DELETE CASCADE
);
...
CREATE TABLE vmware_data_center_zone_map (
  ...
  FOREIGN KEY (vmware_data_center_id) REFERENCES vmware_data_center (id)
    ON DELETE CASCADE
);
...
CREATE TABLE volume_reservation (
  ...
  FOREIGN KEY (vm_reservation_id) REFERENCES vm_reservation (id)
    ON DELETE CASCADE
);
...
FOREIGN KEY (vpc_gateway_id) REFERENCES vpc_gateways (id)
  ON DELETE CASCADE;
...
FOREIGN KEY (vpc_offering_id) REFERENCES vpc_offerings (id);
...
CREATE VIEW account_netstats_view AS
  ...
  GROUP BY `user_statistics`.`account_id`;
...
CREATE VIEW account_vmstats_view AS
  SELECT
    ...
    count(0)                   AS `vmcount`
  FROM `vm_instance`
  WHERE ((`vm_instance`.`vm_type` = 'User') AND isnull(`vm_instance`.`removed`))
  GROUP BY `vm_instance`.`account_id`, `vm_instance`.`state`;

CREATE VIEW free_ip_view AS
  SELECT count(`user_ip_address`.`id`) AS `free_ip`
  FROM (`user_ip_address`
    JOIN `vlan` ON (((`vlan`.`id` = `user_ip_address`.`vlan_db_id`) AND
                     (`vlan`.`vlan_type` = 'VirtualNetwork'))))
  WHERE (`user_ip_address`.`state` = 'Free');
...
CREATE VIEW account_view AS
  ...
  FROM `free_ip_view`
    JOIN ((((((((((((((((((((((((((((((`account`
      JOIN `domain` ON ((`account`.`domain_id` = `domain`.`id`))) LEFT JOIN
      `data_center` ON ((`account`.`default_zone_id` = `data_center`.`id`))) LEFT JOIN
      `account_netstats_view` ON ((`account`.`id` = `account_netstats_view`.`account_id`))) LEFT JOIN
      `resource_limit` `vmlimit` ON (((`account`.`id` = `vmlimit`.`account_id`) AND (`vmlimit`.`type` = 'user_vm')))) LEFT JOIN
      `resource_count` `vmcount` ON (((`account`.`id` = `vmcount`.`account_id`) AND (`vmcount`.`type` = 'user_vm')))) LEFT JOIN
      `account_vmstats_view` `runningvm` ON (((`account`.`id` = `runningvm`.`account_id`) AND (`runningvm`.`state` = 'Running')))) LEFT JOIN
      `account_vmstats_view` `stoppedvm` ON (((`account`.`id` = `stoppedvm`.`account_id`) AND (`stoppedvm`.`state` = 'Stopped')))) LEFT JOIN
      `resource_limit` `iplimit` ON (((`account`.`id` = `iplimit`.`account_id`) AND (`iplimit`.`type` = 'public_ip')))) LEFT JOIN
      `resource_count` `ipcount` ON (((`account`.`id` = `ipcount`.`account_id`) AND (`ipcount`.`type` = 'public_ip')))) LEFT JOIN
      `resource_limit` `volumelimit` ON (((`account`.`id` = `volumelimit`.`account_id`) AND (`volumelimit`.`type` = 'volume')))) LEFT JOIN
      `resource_count` `volumecount` ON (((`account`.`id` = `volumecount`.`account_id`) AND (`volumecount`.`type` = 'volume')))) LEFT JOIN
      `resource_limit` `snapshotlimit` ON (((`account`.`id` = `snapshotlimit`.`account_id`) AND
                                            (`snapshotlimit`.`type` = 'snapshot')))) LEFT JOIN
      `resource_count` `snapshotcount` ON (((`account`.`id` = `snapshotcount`.`account_id`) AND
                                            (`snapshotcount`.`type` = 'snapshot')))) LEFT JOIN
      `resource_limit` `templatelimit` ON (((`account`.`id` = `templatelimit`.`account_id`) AND
                                            (`templatelimit`.`type` = 'template')))) LEFT JOIN
      `resource_count` `templatecount` ON (((`account`.`id` = `templatecount`.`account_id`) AND
                                            (`templatecount`.`type` = 'template')))) LEFT JOIN
      `resource_limit` `vpclimit` ON (((`account`.`id` = `vpclimit`.`account_id`) AND (`vpclimit`.`type` = 'vpc')))) LEFT JOIN
      `resource_count` `vpccount` ON (((`account`.`id` = `vpccount`.`account_id`) AND (`vpccount`.`type` = 'vpc')))) LEFT JOIN
      `resource_limit` `projectlimit` ON (((`account`.`id` = `projectlimit`.`account_id`) AND (`projectlimit`.`type` = 'project')))) LEFT JOIN
      `resource_count` `projectcount` ON (((`account`.`id` = `projectcount`.`account_id`) AND (`projectcount`.`type` = 'project')))) LEFT JOIN
      `resource_limit` `networklimit` ON (((`account`.`id` = `networklimit`.`account_id`) AND (`networklimit`.`type` = 'network')))) LEFT JOIN
      `resource_count` `networkcount` ON (((`account`.`id` = `networkcount`.`account_id`) AND (`networkcount`.`type` = 'network')))) LEFT JOIN
      `resource_limit` `cpulimit` ON (((`account`.`id` = `cpulimit`.`account_id`) AND (`cpulimit`.`type` = 'cpu')))) LEFT JOIN
      `resource_count` `cpucount` ON (((`account`.`id` = `cpucount`.`account_id`) AND (`cpucount`.`type` = 'cpu')))) LEFT JOIN
      `resource_limit` `memorylimit` ON (((`account`.`id` = `memorylimit`.`account_id`) AND (`memorylimit`.`type` = 'memory')))) LEFT JOIN
      `resource_count` `memorycount` ON (((`account`.`id` = `memorycount`.`account_id`) AND (`memorycount`.`type` = 'memory')))) LEFT JOIN
      `resource_limit` `primary_storage_limit` ON (((`account`.`id` = `primary_storage_limit`.`account_id`) AND
                                                    (`primary_storage_limit`.`type` = 'primary_storage')))) LEFT JOIN
      `resource_count` `primary_storage_count` ON (((`account`.`id` = `primary_storage_count`.`account_id`) AND
                                                    (`primary_storage_count`.`type` = 'primary_storage')))) LEFT JOIN
      `resource_limit` `secondary_storage_limit` ON (((`account`.`id` = `secondary_storage_limit`.`account_id`) AND
                                                      (`secondary_storage_limit`.`type` = 'secondary_storage')))) LEFT JOIN
      `resource_count` `secondary_storage_count` ON (((`account`.`id` = `secondary_storage_count`.`account_id`) AND
                                                      (`secondary_storage_count`.`type` = 'secondary_storage')))) LEFT JOIN `async_job`
        ON (((`async_job`.`instance_id` = `account`.`id`) AND (`async_job`.`instance_type` = 'Account')
...
CREATE VIEW affinity_group_view AS
  ...
  FROM ((((((`affinity_group`
    JOIN `account` ON ((`affinity_group`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`affinity_group`.`domain_id` = `domain`.`id`))) LEFT JOIN `projects`
      ON ((`projects`.`project_account_id` = `account`.`id`))) LEFT JOIN `affinity_group_vm_map`
      ON ((`affinity_group`.`id` = `affinity_group_vm_map`.`affinity_group_id`))) LEFT JOIN
    `vm_instance` ON ((`vm_instance`.`id` = `affinity_group_vm_map`.`instance_id`))) LEFT JOIN
    `user_vm` ON ((`user_vm`.`id` = `vm_instance`.`id`)));
...
CREATE VIEW async_job_view AS
  SELECT
    ...
    (CASE WHEN (`async_job`.`instance_type` = 'Volume')
     ...
     WHEN ((`async_job`.`instance_type` = 'VirtualMachine') OR
           (`async_job`.`instance_type` = 'ConsoleProxy') OR (`async_job`.`instance_type` = 'SystemVm')
     ...
     WHEN (`async_job`.`instance_type` = 'AutoScalePolicy')
       THEN `autoscale_policies`.`uuid`
     WHEN (`async_job`.`instance_type` = 'AutoScaleVmProfile')
       THEN `autoscale_vmprofiles`.`uuid`
     WHEN (`async_job`.`instance_type` = 'AutoScaleVmGroup')
       THEN `autoscale_vmgroups`.`uuid`
     ELSE NULL END)                  AS `instance_uuid`
  FROM ((((((((((((((((((((((((`async_job`
    LEFT JOIN `account` ON ((`async_job`.`account_id` = `account`.`id`))) LEFT JOIN
    `domain` ON ((`domain`.`id` = `account`.`domain_id`))) LEFT JOIN `user` ON ((`async_job`.`user_id` = `user`.`id`))) LEFT JOIN
    `volumes` ON ((`async_job`.`instance_id` = `volumes`.`id`))) LEFT JOIN `vm_template` ON ((`async_job`.`instance_id` = `vm_template`.`id`))) LEFT JOIN `vm_instance`
      ON ((`async_job`.`instance_id` = `vm_instance`.`id`))) LEFT JOIN `snapshots` ON ((`async_job`.`instance_id` = `snapshots`.`id`))) LEFT JOIN `host`
      ON ((`async_job`.`instance_id` = `host`.`id`))) LEFT JOIN
    `storage_pool` ON ((`async_job`.`instance_id` = `storage_pool`.`id`))) LEFT JOIN `user_ip_address` ON ((`async_job`.`instance_id` = `user_ip_address`.`id`))) LEFT JOIN
    `security_group` ON ((`async_job`.`instance_id` = `security_group`.`id`))) LEFT JOIN `physical_network` ON ((`async_job`.`instance_id` = `physical_network`.`id`))) LEFT JOIN
    `physical_network_traffic_types` ON ((`async_job`.`instance_id` = `physical_network_traffic_types`.`id`))) LEFT JOIN
    `physical_network_service_providers` ON ((`async_job`.`instance_id` = `physical_network_service_providers`.`id`))) LEFT JOIN
    `firewall_rules` ON ((`async_job`.`instance_id` = `firewall_rules`.`id`))) LEFT JOIN
    `account` `acct` ON ((`async_job`.`instance_id` = `acct`.`id`))) LEFT JOIN `user` `us` ON ((`async_job`.`instance_id` = `us`.`id`))) LEFT JOIN
    `static_routes` ON ((`async_job`.`instance_id` = `static_routes`.`id`))) LEFT JOIN `vpc_gateways` ON ((`async_job`.`instance_id` = `vpc_gateways`.`id`))) LEFT JOIN `counter`
      ON ((`async_job`.`instance_id` = `counter`.`id`))) LEFT JOIN `conditions` ON ((`async_job`.`instance_id` = `conditions`.`id`))) LEFT JOIN `autoscale_policies`
      ON ((`async_job`.`instance_id` = `autoscale_policies`.`id`))) LEFT JOIN
    `autoscale_vmprofiles` ON ((`async_job`.`instance_id` = `autoscale_vmprofiles`.`id`))) LEFT JOIN
    `autoscale_vmgroups` ON ((`async_job`.`instance_id` = `autoscale_vmgroups`.`id`)));

CREATE VIEW data_center_view AS
  SELECT
    `data_center`.`id`                        AS `id`,
    `data_center`.`uuid`                      AS `uuid`,
    `data_center`.`name`                      AS `name`,
    `data_center`.`is_security_group_enabled` AS `is_security_group_enabled`,
    `data_center`.`is_local_storage_enabled`  AS `is_local_storage_enabled`,
    `data_center`.`description`               AS `description`,
    `data_center`.`dns1`                      AS `dns1`,
    `data_center`.`dns2`                      AS `dns2`,
    `data_center`.`ip6_dns1`                  AS `ip6_dns1`,
    `data_center`.`ip6_dns2`                  AS `ip6_dns2`,
    `data_center`.`internal_dns1`             AS `internal_dns1`,
    `data_center`.`internal_dns2`             AS `internal_dns2`,
    `data_center`.`guest_network_cidr`        AS `guest_network_cidr`,
    `data_center`.`domain`                    AS `domain`,
    `data_center`.`networktype`               AS `networktype`,
    `data_center`.`allocation_state`          AS `allocation_state`,
    `data_center`.`zone_token`                AS `zone_token`,
    `data_center`.`dhcp_provider`             AS `dhcp_provider`,
    `data_center`.`removed`                   AS `removed`,
    `domain`.`id`                             AS `domain_id`,
    `domain`.`uuid`                           AS `domain_uuid`,
    `domain`.`name`                           AS `domain_name`,
    `domain`.`path`                           AS `domain_path`,
    `dedicated_resources`.`affinity_group_id` AS `affinity_group_id`,
    `dedicated_resources`.`account_id`        AS `account_id`,
    `affinity_group`.`uuid`                   AS `affinity_group_uuid`
  FROM (((`data_center`
    LEFT JOIN `domain` ON ((`data_center`.`domain_id` = `domain`.`id`))) LEFT JOIN
    `dedicated_resources` ON ((`data_center`.`id` = `dedicated_resources`.`data_center_id`))) LEFT JOIN
    `affinity_group` ON ((`dedicated_resources`.`affinity_group_id` = `affinity_group`.`id`)));
...
CREATE VIEW domain_router_view AS
  ...
  FROM ((((((((((((((`domain_router`
    JOIN `vm_instance` ON ((`vm_instance`.`id` = `domain_router`.`id`))) JOIN `account` ON ((`vm_instance`.`account_id` = `account`.`id`))) JOIN `domain`
      ON ((`vm_instance`.`domain_id` = `domain`.`id`))) LEFT JOIN
    `host_pod_ref` ON ((`vm_instance`.`pod_id` = `host_pod_ref`.`id`))) LEFT JOIN `projects` ON ((`projects`.`project_account_id` = `account`.`id`))) LEFT JOIN `data_center`
      ON ((`vm_instance`.`data_center_id` = `data_center`.`id`))) LEFT JOIN `host` ON ((`vm_instance`.`host_id` = `host`.`id`))) LEFT JOIN
    `vm_template` ON ((`vm_instance`.`vm_template_id` = `vm_template`.`id`))) LEFT JOIN `service_offering`
      ON ((`vm_instance`.`service_offering_id` = `service_offering`.`id`))) LEFT JOIN
    `disk_offering` ON ((`vm_instance`.`service_offering_id` = `disk_offering`.`id`))) LEFT JOIN
    `nics` ON (((`vm_instance`.`id` = `nics`.`instance_id`) AND isnull(`nics`.`removed`)))) LEFT JOIN
    `networks` ON ((`nics`.`network_id` = `networks`.`id`))) LEFT JOIN `vpc` ON (((`domain_router`.`vpc_id` = `vpc`.`id`) AND isnull(`vpc`.`removed`)))) LEFT JOIN
    `async_job` ON (((`async_job`.`instance_id` = `vm_instance`.`id`) AND
                     (`async_job`.`instance_type` = 'DomainRouter') AND
...
CREATE VIEW domain_view AS
  SELECT
    ...
    `secondary_storage_count`.`count` AS `secondaryStorageTotal`
  FROM ((((((((((((((((((((((((`domain`
    LEFT JOIN `resource_limit` `vmlimit` ON (((`domain`.`id` = `vmlimit`.`domain_id`) AND (`vmlimit`.`type` = 'user_vm')))) LEFT JOIN
    `resource_count` `vmcount` ON (((`domain`.`id` = `vmcount`.`domain_id`) AND (`vmcount`.`type` = 'user_vm')))) LEFT JOIN
    `resource_limit` `iplimit` ON (((`domain`.`id` = `iplimit`.`domain_id`) AND (`iplimit`.`type` = 'public_ip')))) LEFT JOIN
    `resource_count` `ipcount` ON (((`domain`.`id` = `ipcount`.`domain_id`) AND (`ipcount`.`type` = 'public_ip')))) LEFT JOIN
    `resource_limit` `volumelimit` ON (((`domain`.`id` = `volumelimit`.`domain_id`) AND (`volumelimit`.`type` = 'volume')))) LEFT JOIN
    `resource_count` `volumecount` ON (((`domain`.`id` = `volumecount`.`domain_id`) AND (`volumecount`.`type` = 'volume')))) LEFT JOIN
    `resource_limit` `snapshotlimit` ON (((`domain`.`id` = `snapshotlimit`.`domain_id`) AND (`snapshotlimit`.`type` = 'snapshot')))) LEFT JOIN
    `resource_count` `snapshotcount` ON (((`domain`.`id` = `snapshotcount`.`domain_id`) AND (`snapshotcount`.`type` = 'snapshot')))) LEFT JOIN
    `resource_limit` `templatelimit` ON (((`domain`.`id` = `templatelimit`.`domain_id`) AND (`templatelimit`.`type` = 'template')))) LEFT JOIN
    `resource_count` `templatecount` ON (((`domain`.`id` = `templatecount`.`domain_id`) AND (`templatecount`.`type` = 'template')))) LEFT JOIN
    `resource_limit` `vpclimit` ON (((`domain`.`id` = `vpclimit`.`domain_id`) AND (`vpclimit`.`type` = 'vpc')))) LEFT JOIN
    `resource_count` `vpccount` ON (((`domain`.`id` = `vpccount`.`domain_id`) AND (`vpccount`.`type` = 'vpc')))) LEFT JOIN
    `resource_limit` `projectlimit` ON (((`domain`.`id` = `projectlimit`.`domain_id`) AND (`projectlimit`.`type` = 'project')))) LEFT JOIN
    `resource_count` `projectcount` ON (((`domain`.`id` = `projectcount`.`domain_id`) AND (`projectcount`.`type` = 'project')))) LEFT JOIN
    `resource_limit` `networklimit` ON (((`domain`.`id` = `networklimit`.`domain_id`) AND (`networklimit`.`type` = 'network')))) LEFT JOIN
    `resource_count` `networkcount` ON (((`domain`.`id` = `networkcount`.`domain_id`) AND (`networkcount`.`type` = 'network')))) LEFT JOIN
    `resource_limit` `cpulimit` ON (((`domain`.`id` = `cpulimit`.`domain_id`) AND (`cpulimit`.`type` = 'cpu')))) LEFT JOIN
    `resource_count` `cpucount` ON (((`domain`.`id` = `cpucount`.`domain_id`) AND (`cpucount`.`type` = 'cpu')))) LEFT JOIN
    `resource_limit` `memorylimit` ON (((`domain`.`id` = `memorylimit`.`domain_id`) AND (`memorylimit`.`type` = 'memory')))) LEFT JOIN
    `resource_count` `memorycount` ON (((`domain`.`id` = `memorycount`.`domain_id`) AND (`memorycount`.`type` = 'memory')))) LEFT JOIN
    `resource_limit` `primary_storage_limit` ON (((`domain`.`id` = `primary_storage_limit`.`domain_id`) AND
                                                  (`primary_storage_limit`.`type` = 'primary_storage')))) LEFT JOIN
    `resource_count` `primary_storage_count` ON (((`domain`.`id` = `primary_storage_count`.`domain_id`) AND
                                                  (`primary_storage_count`.`type` = 'primary_storage')))) LEFT JOIN
    `resource_limit` `secondary_storage_limit` ON (((`domain`.`id` = `secondary_storage_limit`.`domain_id`) AND
                                                    (`secondary_storage_limit`.`type` = 'secondary_storage')))) LEFT JOIN
    `resource_count` `secondary_storage_count` ON (((`domain`.`id` = `secondary_storage_count`.`domain_id`) AND
...
CREATE VIEW event_view AS
  SELECT
    ...
    `projects`.`name`        AS `project_name`
  FROM (((((`event`
    JOIN `account` ON ((`event`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`event`.`domain_id` = `domain`.`id`))) JOIN
    `user` ON ((`event`.`user_id` = `user`.`id`))) LEFT JOIN `projects` ON ((`projects`.`project_account_id` = `event`.`account_id`))) LEFT JOIN `event` `eve`
      ON ((`event`.`start_id` = `eve`.`id`)));
...
CREATE VIEW host_view AS
  SELECT
    ...
    `host`.`disconnected`          AS `disconnected`,
    `host`.`type`                  AS `type`,
    `host`.`private_ip_address`    AS `private_ip_address`,
    `host`.`version`               AS `version`,
    `host`.`hypervisor_type`       AS `hypervisor_type`,
    `host`.`hypervisor_version`    AS `hypervisor_version`,
    `host`.`capabilities`          AS `capabilities`,
    `host`.`last_ping`             AS `last_ping`,
    `host`.`created`               AS `created`,
    `host`.`removed`               AS `removed`,
    `host`.`resource_state`        AS `resource_state`,
    `host`.`mgmt_server_id`        AS `mgmt_server_id`,
    `host`.`cpu_sockets`           AS `cpu_sockets`,
    `host`.`cpus`                  AS `cpus`,
    `host`.`speed`                 AS `speed`,
    `host`.`ram`                   AS `ram`,
    ...
    `host_tags`.`tag`              AS `tag`,
    `guest_os_category`.`id`       AS `guest_os_category_id`,
    `guest_os_category`.`uuid`     AS `guest_os_category_uuid`,
    `guest_os_category`.`name`     AS `guest_os_category_name`,
    ...
    `async_job`.`account_id`       AS `job_account_id`
  FROM (((((((((`host`
    LEFT JOIN `cluster` ON ((`host`.`cluster_id` = `cluster`.`id`))) LEFT JOIN
    `data_center` ON ((`host`.`data_center_id` = `data_center`.`id`))) LEFT JOIN
    `host_pod_ref` ON ((`host`.`pod_id` = `host_pod_ref`.`id`))) LEFT JOIN
    `host_details` ON (((`host`.`id` = `host_details`.`host_id`) AND
                        (`host_details`.`name` = 'guest.os.category.id')))) LEFT JOIN
    `guest_os_category` ON ((`guest_os_category`.`id` = cast(`host_details`.`value` AS UNSIGNED)))) LEFT JOIN
    `host_tags` ON ((`host_tags`.`host_id` = `host`.`id`))) LEFT JOIN
    `op_host_capacity` `mem_caps` ON (((`host`.`id` = `mem_caps`.`host_id`) AND (`mem_caps`.`capacity_type` = 0)))) LEFT JOIN
    `op_host_capacity` `cpu_caps` ON (((`host`.`id` = `cpu_caps`.`host_id`) AND (`cpu_caps`.`capacity_type` = 1)))) LEFT JOIN
    `async_job` ON (((`async_job`.`instance_id` = `host`.`id`) AND (`async_job`.`instance_type` = 'Host')
                     AND (`async_job`.`job_status` = 0))));
...
CREATE VIEW image_store_view AS
  ...
  FROM ((`image_store`
    LEFT JOIN `data_center` ON ((`image_store`.`data_center_id` = `data_center`.`id`))) LEFT JOIN
    `image_store_details` ON ((`image_store_details`.`store_id` = `image_store`.`id`)));
...
CREATE VIEW instance_group_view AS
  SELECT
    ...
    `projects`.`name`          AS `project_name`
  FROM (((`instance_group`
    JOIN `account` ON ((`instance_group`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`account`.`domain_id` = `domain`.`id`))) LEFT JOIN `projects`
      ON ((`projects`.`project_account_id` = `instance_group`.`account_id`)));
...
CREATE VIEW project_account_view AS
  ...
  FROM (((`project_account`
    JOIN `account` ON ((`project_account`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`account`.`domain_id` = `domain`.`id`))) JOIN `projects`
      ON ((`projects`.`id` = `project_account`.`project_id`)));
...
CREATE VIEW project_invitation_view AS
  ...
  FROM (((`project_invitations`
    LEFT JOIN `account` ON ((`project_invitations`.`account_id` = `account`.`id`))) LEFT JOIN
    `domain` ON ((`project_invitations`.`domain_id` = `domain`.`id`))) LEFT JOIN
    `projects` ON ((`projects`.`id` = `project_invitations`.`project_id`)));
...
CREATE VIEW project_view AS
  ...
  FROM (((((`projects`
    JOIN `domain` ON ((`projects`.`domain_id` = `domain`.`id`))) JOIN `project_account` ON (((`projects`.`id` = `project_account`.`project_id`) AND
                                                                                             (`project_account`.`account_role` = 'Admin')))) JOIN `account`
      ON ((`account`.`id` = `project_account`.`account_id`))) LEFT JOIN
    `resource_tags` ON (((`resource_tags`.`resource_id` = `projects`.`id`) AND
                         (`resource_tags`.`resource_type` = 'Project')))) LEFT JOIN `project_account` `pacct` ON ((`projects`.`id` = `pacct`.`project_id`)));
...
CREATE VIEW resource_tag_view AS
  SELECT
    ...
    `projects`.`name`               AS `project_name`
  FROM (((`resource_tags`
    JOIN `account` ON ((`resource_tags`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`resource_tags`.`domain_id` = `domain`.`id`))) LEFT JOIN `projects`
      ON ((`projects`.`project_account_id` = `resource_tags`.`account_id`)));
...
CREATE VIEW security_group_view AS
  SELECT
    ...
    `async_job`.`account_id`                   AS `job_account_id`
  FROM ((((((`security_group`
    LEFT JOIN `security_group_rule` ON ((`security_group`.`id` = `security_group_rule`.`security_group_id`))) JOIN `account`
      ON ((`security_group`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`security_group`.`domain_id` = `domain`.`id`))) LEFT JOIN
    `projects` ON ((`projects`.`project_account_id` = `security_group`.`account_id`))) LEFT JOIN
    `resource_tags` ON (((`resource_tags`.`resource_id` = `security_group`.`id`) AND
                         (`resource_tags`.`resource_type` = 'SecurityGroup')))) LEFT JOIN
    `async_job` ON (((`async_job`.`instance_id` = `security_group`.`id`) AND
                     (`async_job`.`instance_type` = 'SecurityGroup') AND
                     (`async_job`.`job_status` = 0))));
...
CREATE VIEW service_offering_view AS
  ...
  FROM ((`service_offering`
    JOIN `disk_offering` ON ((`service_offering`.`id` = `disk_offering`.`id`))) LEFT JOIN
    `domain` ON ((`disk_offering`.`domain_id` = `domain`.`id`)))
  WHERE (`disk_offering`.`state` = 'Active');
...
CREATE VIEW storage_pool_view AS
  SELECT
    ...
    `host_pod_ref`.`uuid`                  AS `pod_uuid`,
    `host_pod_ref`.`name`                  AS `pod_name`,
    `storage_pool_details`.`name`          AS `tag`,
    ...
    `async_job`.`account_id`               AS `job_account_id`
  FROM ((((((`storage_pool`
    LEFT JOIN `cluster` ON ((`storage_pool`.`cluster_id` = `cluster`.`id`))) LEFT JOIN
    `data_center` ON ((`storage_pool`.`data_center_id` = `data_center`.`id`))) LEFT JOIN
    `host_pod_ref` ON ((`storage_pool`.`pod_id` = `host_pod_ref`.`id`))) LEFT JOIN
    `storage_pool_details` ON (((`storage_pool_details`.`pool_id` = `storage_pool`.`id`) AND
                                (`storage_pool_details`.`value` = 'true')))) LEFT JOIN
    `op_host_capacity` ON (((`storage_pool`.`id` = `op_host_capacity`.`host_id`) AND
                            (`op_host_capacity`.`capacity_type` IN (3, 9))))) LEFT JOIN
    `async_job` ON (((`async_job`.`instance_id` = `storage_pool`.`id`) AND
                     (`async_job`.`instance_type` = 'StoragePool') AND
                     (`async_job`.`job_status` = 0))));

CREATE VIEW storage_tag_view AS
  SELECT
    `storage_pool_details`.`id`      AS `id`,
    `storage_pool_details`.`pool_id` AS `pool_id`,
    `storage_pool_details`.`name`    AS `name`
  FROM `storage_pool_details`
  WHERE (`storage_pool_details`.`value` = 'true');
...
CREATE VIEW template_view AS
  ...
  FROM ((((((((((((`vm_template`
    JOIN `guest_os` ON ((`guest_os`.`id` = `vm_template`.`guest_os_id`))) JOIN `account` ON ((`account`.`id` = `vm_template`.`account_id`))) JOIN `domain`
      ON ((`domain`.`id` = `account`.`domain_id`))) LEFT JOIN
    `projects` ON ((`projects`.`project_account_id` = `account`.`id`))) LEFT JOIN `vm_template_details` ON ((`vm_template_details`.`template_id` = `vm_template`.`id`))) LEFT JOIN
    `vm_template` `source_template` ON ((`source_template`.`id` = `vm_template`.`source_template_id`))) LEFT JOIN `template_store_ref`
      ON (((`template_store_ref`.`template_id` = `vm_template`.`id`) AND
           (`template_store_ref`.`store_role` = 'Image') AND
           (`template_store_ref`.`destroyed` = 0)))) LEFT JOIN `image_store` ON ((isnull(`image_store`.`removed`) AND (`template_store_ref`.`store_id` IS NOT NULL) AND
                                                                                  (`image_store`.`id` = `template_store_ref`.`store_id`)))) LEFT JOIN
    `template_zone_ref` ON (((`template_zone_ref`.`template_id` = `vm_template`.`id`) AND
                             isnull(`template_store_ref`.`store_id`) AND
                             isnull(`template_zone_ref`.`removed`)))) LEFT JOIN `data_center` ON (((`image_store`.`data_center_id` = `data_center`.`id`) OR
                                                                                                   (`template_zone_ref`.`zone_id` = `data_center`.`id`)))) LEFT JOIN
    `launch_permission` ON ((`launch_permission`.`template_id` = `vm_template`.`id`))) LEFT JOIN `resource_tags` ON (((`resource_tags`.`resource_id` = `vm_template`.`id`) AND
...
CREATE VIEW user_view AS
  SELECT
    ...
    `user`.`secret_key`               AS `secret_key`,
    `user`.`created`                  AS `created`,
    `user`.`removed`                  AS `removed`,
    `user`.`timezone`                 AS `timezone`,
    `user`.`registration_token`       AS `registration_token`,
    `user`.`is_registered`            AS `is_registered`,
    `user`.`incorrect_login_attempts` AS `incorrect_login_attempts`,
    `user`.`default`                  AS `default`,
    `account`.`id`                    AS `account_id`,
    `account`.`uuid`                  AS `account_uuid`,
    `account`.`account_name`          AS `account_name`,
    `account`.`type`                  AS `account_type`,
    ...
    `async_job`.`account_id`          AS `job_account_id`
  FROM (((`user`
    JOIN `account` ON ((`user`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`account`.`domain_id` = `domain`.`id`))) LEFT JOIN `async_job`
      ON (((`async_job`.`instance_id` = `user`.`id`) AND (`async_job`.`instance_type` = 'User')
           AND (`async_job`.`job_status` = 0))));
...
CREATE VIEW user_vm_view AS
  SELECT
    ...
    (CASE WHEN isnull(`service_offering`.`cpu`)
      THEN `custom_cpu`.`value`
     ELSE `service_offering`.`cpu` END)       AS `cpu`,
    (CASE WHEN isnull(`service_offering`.`speed`)
      THEN `custom_speed`.`value`
     ELSE `service_offering`.`speed` END)     AS `speed`,
    (CASE WHEN isnull(`service_offering`.`ram_size`)
      THEN `custom_ram_size`.`value`
     ELSE `service_offering`.`ram_size` END)  AS `ram_size`,
  ...
  FROM ((((((((((((((((((((((((((((((((`user_vm`
    JOIN `vm_instance` ON (((`vm_instance`.`id` = `user_vm`.`id`) AND isnull(`vm_instance`.`removed`)))) JOIN
    `account` ON ((`vm_instance`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`vm_instance`.`domain_id` = `domain`.`id`))) LEFT JOIN `guest_os`
      ON ((`vm_instance`.`guest_os_id` = `guest_os`.`id`))) LEFT JOIN
    `host_pod_ref` ON ((`vm_instance`.`pod_id` = `host_pod_ref`.`id`))) LEFT JOIN `projects` ON ((`projects`.`project_account_id` = `account`.`id`))) LEFT JOIN
    `instance_group_vm_map` ON ((`vm_instance`.`id` = `instance_group_vm_map`.`instance_id`))) LEFT JOIN
    `instance_group` ON ((`instance_group_vm_map`.`group_id` = `instance_group`.`id`))) LEFT JOIN `data_center`
      ON ((`vm_instance`.`data_center_id` = `data_center`.`id`))) LEFT JOIN `host` ON ((`vm_instance`.`host_id` = `host`.`id`))) LEFT JOIN
    `vm_template` ON ((`vm_instance`.`vm_template_id` = `vm_template`.`id`))) LEFT JOIN `vm_template` `iso` ON ((`iso`.`id` = `user_vm`.`iso_id`))) LEFT JOIN `service_offering`
      ON ((`vm_instance`.`service_offering_id` = `service_offering`.`id`))) LEFT JOIN
    `disk_offering` `svc_disk_offering` ON ((`vm_instance`.`service_offering_id` = `svc_disk_offering`.`id`))) LEFT JOIN `disk_offering`
      ON ((`vm_instance`.`disk_offering_id` = `disk_offering`.`id`))) LEFT JOIN `volumes` ON ((`vm_instance`.`id` = `volumes`.`instance_id`))) LEFT JOIN `storage_pool`
      ON ((`volumes`.`pool_id` = `storage_pool`.`id`))) LEFT JOIN `security_group_vm_map` ON ((`vm_instance`.`id` = `security_group_vm_map`.`instance_id`))) LEFT JOIN
    `security_group` ON ((`security_group_vm_map`.`security_group_id` = `security_group`.`id`))) LEFT JOIN
    `nics` ON (((`vm_instance`.`id` = `nics`.`instance_id`) AND isnull(`nics`.`removed`)))) LEFT JOIN
    `networks` ON ((`nics`.`network_id` = `networks`.`id`))) LEFT JOIN `vpc` ON (((`networks`.`vpc_id` = `vpc`.`id`) AND isnull(`vpc`.`removed`)))) LEFT JOIN
    `user_ip_address` ON ((`user_ip_address`.`vm_id` = `vm_instance`.`id`))) LEFT JOIN
    `user_vm_details` `ssh_details` ON (((`ssh_details`.`vm_id` = `vm_instance`.`id`) AND
                                         (`ssh_details`.`name` = 'SSH.PublicKey')))) LEFT JOIN
    `ssh_keypairs` ON (((`ssh_keypairs`.`public_key` = `ssh_details`.`value`) AND
                        (`ssh_keypairs`.`account_id` = `account`.`id`)))) LEFT JOIN
    `resource_tags` ON (((`resource_tags`.`resource_id` = `vm_instance`.`id`) AND
                         (`resource_tags`.`resource_type` = 'UserVm')))) LEFT JOIN `async_job` ON (((`async_job`.`instance_id` = `vm_instance`.`id`) AND
                                                                                                    (`async_job`.`instance_type` = 'VirtualMachine') AND
                                                                                                    (`async_job`.`job_status` = 0)))) LEFT JOIN `affinity_group_vm_map`
      ON ((`vm_instance`.`id` = `affinity_group_vm_map`.`instance_id`))) LEFT JOIN
    `affinity_group` ON ((`affinity_group_vm_map`.`affinity_group_id` = `affinity_group`.`id`))) LEFT JOIN
    `user_vm_details` `custom_cpu` ON (((`custom_cpu`.`vm_id` = `vm_instance`.`id`) AND (`custom_cpu`.`name` = 'CpuNumber')))) LEFT JOIN
    `user_vm_details` `custom_speed` ON (((`custom_speed`.`vm_id` = `vm_instance`.`id`) AND (`custom_speed`.`name` = 'CpuSpeed')))) LEFT JOIN
    `user_vm_details` `custom_ram_size` ON (((`custom_ram_size`.`vm_id` = `vm_instance`.`id`) AND (`custom_ram_size`.`name` = 'memory'))));
...
CREATE VIEW volume_view AS
  SELECT
    ...
    `async_job`.`account_id`            AS `job_account_id`
  FROM ((((((((((((((`volumes`
    JOIN `account` ON ((`volumes`.`account_id` = `account`.`id`))) JOIN `domain` ON ((`volumes`.`domain_id` = `domain`.`id`))) LEFT JOIN `projects`
      ON ((`projects`.`project_account_id` = `account`.`id`))) LEFT JOIN
    `data_center` ON ((`volumes`.`data_center_id` = `data_center`.`id`))) LEFT JOIN `vm_instance` ON ((`volumes`.`instance_id` = `vm_instance`.`id`))) LEFT JOIN `user_vm`
      ON ((`user_vm`.`id` = `vm_instance`.`id`))) LEFT JOIN
    `volume_store_ref` ON ((`volumes`.`id` = `volume_store_ref`.`volume_id`))) LEFT JOIN `disk_offering` ON ((`volumes`.`disk_offering_id` = `disk_offering`.`id`))) LEFT JOIN
    `storage_pool` ON ((`volumes`.`pool_id` = `storage_pool`.`id`))) LEFT JOIN `cluster` ON ((`storage_pool`.`cluster_id` = `cluster`.`id`))) LEFT JOIN
    `vm_template` ON ((`volumes`.`template_id` = `vm_template`.`id`))) LEFT JOIN `vm_template` `iso` ON ((`iso`.`id` = `volumes`.`iso_id`))) LEFT JOIN `resource_tags`
      ON (((`resource_tags`.`resource_id` = `volumes`.`id`) AND
           (`resource_tags`.`resource_type` = 'Volume')))) LEFT JOIN `async_job` ON (((`async_job`.`instance_id` = `volumes`.`id`) AND (`async_job`.`instance_type` = 'Volume')
                                                                                      AND (`async_job`.`job_status` = 0))));
